...
# Глава 2: Моделирование вашего кода: коммуникация. Последовательные процессы
...
Как мы рассмотрели в разделе «Горутины» на странице 37, мы знаем, что горутины дешевы и просты в создании; это одна из вещей, которая делает Go таким продуктивным языком.

Среда выполнения обрабатывает мультиплексирование горутинов в любое количество потоков операционной системы, так что нам не часто приходится беспокоиться об этом уровне абстракции. Но
они требуют ресурсов, и горутины не являются мусором, собираемым средой выполнения, поэтому
независимо от того, насколько мал их объем памяти, мы не хотим оставлять их лежать
в нашем процессе. Так как же нам обеспечить их очистку?

Давайте начнем с самого начала и подумаем об этом шаг за шагом: зачем существует горутина? В Главе 2 мы установили, что горутины представляют собой единицы работы, которые
могут или не могут выполняться параллельно друг с другом. У goroutine есть несколько путей завершения:
- Когда она завершила свою работу.
- Когда она не может продолжить свою работу из-за неустранимой ошибки.
- Когда ей говорят прекратить работу.

Мы получаем первые два пути бесплатно — эти пути — ваш алгоритм — но что насчет отмены работы? Это оказывается самым важным из-за сетевого эффекта: если вы начали goroutine, она, скорее всего, сотрудничает с несколькими другими goroutine каким-то организованным образом. Мы даже могли бы представить эту взаимосвязь в виде графика: следует ли дочерней goroutine продолжать выполнение, может быть основано на знании состояния многих других goroutine. Родительская goroutine (часто главная goroutine) с этим полным контекстным знанием должна иметь возможность сказать своим дочерним goroutine завершиться. Мы продолжим рассматривать взаимозависимость горутин в масштабах в следующей главе, но сейчас давайте рассмотрим, как гарантировать, что одна дочерняя горутина будет гарантированно очищена. Давайте начнем с простого примера утечки горутины:
...
Здесь мы видим, что основная горутина передает нулевой канал в doWork. Следовательно, канал строк никогда не получит никаких строк, записанных в него, и горутина,
содержащая doWork, останется в памяти на протяжении всего срока службы этого процесса (мы бы даже заблокировали, если бы объединили горутину в doWork и основную горутину).

В этом примере срок службы процесса очень короткий, но в реальной программе горутины могут легко запускаться в начале долгоживущей программы. В худшем случае основная горутина может продолжать раскручивать горутины на протяжении всей своей жизни,
вызывая неконтролируемое использование памяти.

Способ успешного смягчения этого — установить сигнал между родительской горутиной и ее дочерними, который позволяет родителю сигнализировать об отмене своим дочерним процессам. По соглашению, этот сигнал обычно является каналом только для чтения с именем done. Родительская горутина передает этот канал дочерней горутине, а затем закрывает канал, когда хочет отменить дочернюю горутину. Вот пример:
...
1) Здесь мы передаем канал done в функцию doWork. По соглашению этот
канал является первым параметром.
2) В этой строке мы видим повсеместно используемый шаблон for-select. Один из наших операторов case проверяет, был ли сигнализирован наш канал done. Если это так, мы
возвращаемся из goroutine.
3) Здесь мы создаем еще одну goroutine, которая отменит goroutine, порожденную в doWork, если пройдет больше одной секунды.
4) Здесь мы соединяем goroutine, порожденную из doWork, с основной goroutine.
...
Вы можете видеть, что несмотря на передачу nil для нашего канала строк, наша горутина все еще
успешно завершается. В отличие от предыдущего примера, в этом примере мы объединяем две
горутины и при этом не получаем тупика. Это потому, что перед тем, как объединить две
горутины, мы создаем третью горутину для отмены горутины в doWork через
секунду. Мы успешно устранили утечку нашей горутины!

Предыдущий пример отлично справляется с случаем получения горутин на канале,
но что, если мы имеем дело с обратной ситуацией: горутина заблокирована при попытке записать значение в канал? Вот быстрый пример, демонстрирующий проблему:
...
```
1) Здесь мы выводим сообщение, когда горутина успешно завершается.
...
Из вывода видно, что отложенный оператор fmt.Println (из defer) никогда не запускается.

После третьей итерации нашего цикла наша goroutine блокируется, пытаясь отправить следующее случайное целое число в канал, из которого больше не считывается. У нас нет способа сообщить производителю, что он может остановиться. Решение, как и в случае с приемником, состоит в том, чтобы предоставить goroutine производителя канал, сообщающий ему о необходимости выхода:
